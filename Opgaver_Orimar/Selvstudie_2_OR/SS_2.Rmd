...
finite_difference <- function(df, input, t, x, v, delta_n_time, delta_m_price, delta_q_vol) {
  delta <- case_when(input == 1 ~ delta_n_time,
                     input == 2 ~ delta_m_price,
                     input == 3 ~ delta_q_vol,
                     input == c(2,2) ~ delta_m_price^2,
                     input == c(3,3) ~ delta_q_vol^2,
                     input == c(2,3) ~ delta_m_price * delta_q_vol)
  if (length(input) == 1) {
    df %>% filter(t == t, x == x, v == v) %>% select(price)
  }
}
...
ex_5 <- finite_difference_approx(n_ = 100, m_ = 10, q_ = 10,
  x_min = 0, x_max = 10, v_min = 0,
  v_max = 10, T = 1, K = 1)
